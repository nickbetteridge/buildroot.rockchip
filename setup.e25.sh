...
if [ $# -lt 1 ]; then
  ...
  BR_REPO_PATH=$1
  BR_DEFCONFIG=''
  BR_RK3568_DEFCONFIG=rk3568_defconfig
  ...
  if [ ! -e $CUSTOM_PATH/configs/$BR_RK3568_DEFCONFIG ]; then
  	echo can\'t find the file $CUSTOM_PATH/configs/$BR_RK3568_DEFCONFIG
  ...
  if [ ! -z $BR_DEFCONFIG ]; then
    ...
    echo "###### DON'T EDIT THIS FILE, EDIT $BR_RK3568_DEFCONFIG INSTEAD" > $CUSTOM_PATH/configs/$BR_DEFCONFIG
    cat $BR_REPO_PATH/configs/$BR_DEFCONFIG >> $CUSTOM_PATH/configs/$BR_DEFCONFIG
    cat $CUSTOM_PATH/configs/$BR_RK3568_DEFCONFIG >> $CUSTOM_PATH/configs/$BR_DEFCONFIG
    make BR2_EXTERNAL=$CUSTOM_PATH $BR_DEFCONFIG
  else
    echo No original defconfig defined, no problem using $BR_RK3568_DEFCONFIG
    make BR2_EXTERNAL=$CUSTOM_PATH $BR_RK3568_DEFCONFIG
  fi
fi
